...
只有当前环境无法访问仓库文件时，才使用“完整 JSON 输出 → 用户手动导入”的兜底路径。
...
无法读取或写入私人文件时，输出一份完整 JSON，而不是补丁。它必须包含当前整张地图、保留所有未修改内容和稳定 id，并把 `meta.revision` 设置为输入 revision 加一。输出前完成同样的结构检查，并明确告诉用户：文件和网站尚未被自动更新，需要手动导入。
